...
from config import BASE_DIR, PUBLICO_LIGAS
from funcoes import obter_dados_historicos, obter_dados_ligas


def _atribuir_pontos_ranking(dados, campo_pontuacao):
    pontos = 20
    for item in dados:
        item[campo_pontuacao] = max(pontos, 0)
        pontos -= 1
    return dados


def _parse_media_publico(valor):
    if isinstance(valor, str):
        return int(valor.replace(".", "").replace(",", ""))
    return valor or 0


def _caminho_publico(codigo_liga, temporada):
    info_liga = PUBLICO_LIGAS.get(codigo_liga)
    if not info_liga:
        return None

    return (
        BASE_DIR
        / "media de publico por tmeporada"
        / info_liga["pasta"]
        / f"media_{info_liga['nome_arquivo']}_{temporada}.json"
    )
...
def calcular_pontuacao_publico(codigo_liga, temporada):
    ...
    """
    caminho_arquivo = _caminho_publico(codigo_liga, temporada)
    ...
    try:
        ...
        for time in dados:
            time["Media_Publico"] = _parse_media_publico(time.get("Media_Publico"))
                 
        dados_ordenados = sorted(dados, key=lambda x: x["Media_Publico"], reverse=True)
        return _atribuir_pontos_ranking(dados_ordenados, "Pontuacao")

    except Exception as erro:
        print(f"Erro ao processar pontuação de público: {erro}")
        return None
...
def calcular_pontuacao_historica(codigo_liga, temporada_atual):
    ...
    return _atribuir_pontos_ranking(media_posicoes_ordenada, "Pontuacao_Historica")
...
def calcular_pontuacao_atual(codigo_liga, temporada_atual):
    ...
    return _atribuir_pontos_ranking(lista_atual, "Pontuacao_Atual")
...
def calcular_valor_patrocinio(codigo_liga, temporada):
    ...
    return sorted(lista_final, key=lambda x: x["Valor_Patrocinio"], reverse=True)
